...
**Personal lane (Lane B) on production: exercised for the first time today, and it found a bug.** At 05:20 UTC the production tables held 0 items, 0 matches, 0 claims and 0 LLM calls; every receipt-to-claim measurement earlier in this log came from the dev deployment. Between 15:40 and 16:03 UTC Tony uploaded his real receipts on production (Home Depot HEIC, the pharmacy receipt, a typed-in ladder), the two seeded retailer receipts were emailed to his production alias, and the loop ran on the production URL: 13 items, 13 LLM calls, 2 confirmed matches (teether 0.98, power bank 0.96), 2 alert emails, 2 Firecrawl remedy scrapes with extracted procedures (6 and 4 steps). The first attempt at recording those matches failed on every retry, which is the 09-22 entry at the end of the log.

**Guards, verified in place:** third-party sends (claim emails to manufacturers) still restricted by the owner-controlled allowlist on both deployments, while sign-in codes and recall alerts go to the account's own address; new accounts capped at 1,000 (`SIGNUP_CAP` in `convex/auth.ts`); `OTP_DEV_FALLBACK` set only on dev; detail scrapes capped at 300 per run; Firecrawl parallelism 2, LLM parallelism 3; per-user intake cap of 20 receipts/day; budget guard at $5/day and $70 total; no `.env` file tracked, secrets only in Convex env.
...
### 2026-09-22 - 09e4905 (the gate finds a bug)
The M10 gate, run eleven days late, earned its place in the plan. With Tony's real receipts on production and the seeded recalled-items receipt emailed to his production alias, intake worked (9 photo/typed items, then 4 emailed items within 3 s of delivery), adjudication worked (6 calls; the teether and power bank both judged matches with rationales naming their model numbers), and then **every match failed to record**: `match:recordMatches` threw "dynamic module import unsupported" at the line that decides whether to schedule the remedy scrape. A `const { needsRemedyScrape } = await import("./remedy")` had been written in M8 (09-09) to sidestep a suspected import cycle; convex-test under vitest resolves dynamic imports happily, so 84 tests stayed green, and the only match created on dev after that commit was the typed-in ladder's possible-match, which skips this branch. No confirmed match had run through the M8 code on any deployment. The same pattern sat in `remedy:backfillRemedyPages`. Both became static imports (there was no cycle: `remedy.ts` never imports `match.ts`), typecheck and tests green, deployed to production at 16:01 UTC, adjudication re-run for the two items by hand: **2 matches created, both alerted, both remedy portals scraped through Firecrawl and extracted** (skiphoprecall.com, recallrtr.com/xopowerbank). Lesson for the log: a test runtime more permissive than the production runtime hides exactly this class of bug, and the only cure is running the real path on the real deployment, which the plan said to do on 09-08.

Also this session: the M16 deep-pass sections above (architecture, module map, Firecrawl proofs, cost table, production snapshot) and the NHTSA copy correction. Next: Tony records the demo on production, then the launch post and the submission.

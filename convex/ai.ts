...
/** One OpenAI Responses API call with strict structured output; retries once
 * on invalid JSON, then throws for the caller to flag human review. */
export async function callStructured(
  ...
  if (!budget.allowed) {
    ...
    throw new Error(`LLM budget guard: ${budget.reason}`);
  }
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    if (refusal) throw new Error(`OpenAI refusal: ${refusal.refusal?.slice(0, 200)}`);
    if (json.status === "incomplete") {
      console.warn(`OpenAI incomplete response for ${call.purpose} (attempt ${attempt})`);
    }
    try {
      return JSON.parse(content?.text ?? "") as Record<string, unknown>;
    } catch {
      if (attempt === 1) throw new Error(`OpenAI returned invalid JSON twice (${call.purpose})`);
    }
  }
